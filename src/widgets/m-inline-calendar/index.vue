  ...
  <div class="inline-calendar" :class="{'is-weekend-highlight': highlightWeekend}">
    ...
    <table>
      <thead v-show="!hideWeekList">
        <tr>
          <th v-for="(week, index) in _weeksList" :key="(week, index)" class="week" :class="`is-week-list-${index}`">{{ week || ('week_day_' + index /* en: week, zh-CN: week */) }}</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(day,k1) in days" :key="(day,k1)">
          <td
          v-for="(child,k2) in day" :key="(child,k2)"
...
<script>
import format from '../m-datetime/format'
import { getDays, zero, isBetween } from './util'
import props from './props'
import calendarMarksMixin from 'src/service/mixins/calendar-marks'
