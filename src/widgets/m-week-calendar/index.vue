    ...
    <swiper
      ...
      loop>
        ...
        <template v-for="(item, index) in [lastWeekList, currentWeekList, nextWeekList]">
          ...
          <swiper-item :data-start="item[0].format" :id="`x000${index}`" :key="(item, index)">
...
import { Swiper, SwiperItem } from '../m-swiper'
import dateFormat from 'src/service/tools/date/format'
