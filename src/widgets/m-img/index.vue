...
import { isSupported, detectWebp } from 'src/service/libs/webp-support'
...
export default {
  ...
  mixins: [uuidMixin],
  created () {
    this.$zk && this.$zk.bus && this.$zk.bus.$on('vux:after-view-enter', this.init)
  },
  methods: {
    init () {
      const _this = this
  ...
  beforeDestroy () {
    this.blazy && this.blazy.destroy()
    this.blazy = null
    this.$zk && this.$zk.bus && this.$zk.bus.$off('vux:after-view-enter', this.init)
  }
}
</script>
